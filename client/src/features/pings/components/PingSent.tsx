...
export const PingSent: React.FC = () => {
  ...
  const { data: pings, isLoading, isError, refetch } = usePingsSent();
    ...
    return (
      <Card className="border-destructive/20 bg-destructive/[2%] text-center p-6 my-4 rounded-xl">
        <CardContent className="flex flex-col items-center gap-4 py-4 select-none">
          ...
          <div className="space-y-1">
            <h4 className="text-body-p font-bold text-foreground">Failed to load sent pings</h4>
            <p className="text-metadata text-muted-foreground leading-normal max-w-xs">
              We encountered a network issue synchronising your sent history. Let's try requesting the data again.
            </p>
          </div>
          <Button 
            variant="outline" 
  ...
  if (!pings || pings.length === 0) {
    return (
      <EmptyState
        icon={Inbox}
        title="No sent pings yet"
        description="Pings allow you to ask targeted questions to pathfinders or expert guides. Reach out to a guide to book sessions or ask questions."
